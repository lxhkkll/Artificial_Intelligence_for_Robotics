...
def calc_dynamic_window(x, config):
    """
    位置空间集合
    :param x:当前位置空间,符号参考硕士论文[_,_,_,V,w]
    :param config:
...
def calc_trajectory(x_init, v, w, config):
    ...
    while time <= config.predict_time:
        x = motion(x, [v, w], config.dt) # 模型解算约束
...
def calc_final_input(x, u, vr, config, goal, ob):
    ...
    trajectory_space = np.array([x])  # 记录搜索所有采样的轨迹，用来画图
    ...
    for v in np.arange(vr[0], vr[1], config.v_reso):
        for w in np.arange(vr[2], vr[3], config.yawrate_reso):
            ...
            trajectory_space = np.vstack((trajectory_space, trajectory))
    ...
    return min_u, best_trajectory, trajectory_space
...
def dwa_control(x, u, config, goal, ob):
    ...
    u, trajectory, trajectory_space = calc_final_input(x, u, vr, config, goal, ob)

    return u, trajectory, trajectory_space
...
def dynamic_obstacle():
    """
    生成多个障碍物,但是不能生成在起点和终点
    :return:
    """
    obstacle = np.random.randint(1, 10, size=(10, 2))
    return obstacle
...
def main():
    ...
    while True:
        ...
        ob = dynamic_obstacle()  # 障碍物初始化

        u, best_trajectory, trajectory_space = dwa_control(x, u, config, goal, ob)
        ...
        # 画出每次前进的结果
        draw_dynamic_search(best_trajectory, x, goal, ob, trajectory_space)
        # draw_path(trajectory, goal, ob, x)
        # check goal，小于机器人半径，则搜索结束
        if math.sqrt((x[0] - goal[0]) ** 2 + (x[1] - goal[1]) ** 2) <= config.robot_radius:
            print("Goal!!")

            break

        # obstacle_time += config.dt

    print("Done")


def draw_dynamic_search(best_trajectory, x, goal, ob, trajectory_space):
    """
    画出动态搜索过程图
    :return:
    """
    ...
    plt.plot(trajectory_space[:, 0], trajectory_space[:, 1], 'r', alpha=0.3)
